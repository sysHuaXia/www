...
function BindSeachList(str) {
    ...
    tStr += '<td height="35" bgcolor="#FF8E8E" width="3%"><div align="center">序号</div></td>';
    tStr += '<td height="35" bgcolor="#FF8E8E" width="12%"><div align="center">客人名</div></td>';
    tStr += '<td height="35" bgcolor="#FF8E8E" width="10%"><div align="center">订单号</div></td>';
    tStr += '<td height="35" bgcolor="#FF8E8E" width="6%" ><div align="center">问题级别</div></td>';
    ...
    tStr += '<td height="35" bgcolor="#FF8E8E" width="12%"><div align="center">关联人</div></td>';
    tStr += '<td height="35" bgcolor="#FF8E8E"><div align="center">审核记录</div></td>';
    ...
    for (var i = 0; i < orders.length - 1 ; i++) {
        ...
        tStr += '<td>' + h + '</td>';
        tStr += '<td><a href="/Order/UpdateOrderShow?oid=' + order[1] + '&version=' + (new Date()).getTime() + '" target="_blank" style="color:#6993E9;font-size:13px;" id="lsh_' + order[1] + '">' + order[2] + '</a></td>';
        tStr += '<td>' + order[3] + '</td>';
        tStr += '<td>' + order[4] + '</td>';
        tStr += '<td>' + order[5] + '</td>';
        tStr += '<td>' + order[6] + '<br><a target="_blank" href="http://www.taobao.com/webww/ww.php?ver=3&touid=' + order[6] + '&siteid=cntaobao&status=1&charset=utf-8"><img border="0" src="http://amos.alicdn.com/realonline.aw?v=2&uid=' + order[6] + '&site=cntaobao&s=1&charset=utf-8" alt="' + order[6] + '" /></a></td>';
        tStr += '<td>' + order[7] + '</td>';
        tStr += '<td align="left">' + order[8] + '<br/><span style="color:#ADADAD">审核人:' + order[12] + '&nbsp;&nbsp;&nbsp;审核时间:' + order[9] + '</span></td>';
        tStr += '<td><span onclick="UpdateAuditRecords(' + order[1] + ')" style="color:#b3beac;">记录</span></td>';
...
//自动加载数据
function BindIniAuditData(AuditRecordsUser, Year, Month, ALevelId, Count) {
    if (AuditRecordsUser == 0 && Year == 0 && Month == 0 && ALevelId == 0) {
        $(".select-no").show();
    } else {
        $(".select-no").hide();
    }
...
function BindOperation() {
    $.ajax({
        type: "GET",
        url: "/AuditRecords/GetAuditRecordsUser",
        success: function (sesponseTest) {
            if (sesponseTest != "0") {
