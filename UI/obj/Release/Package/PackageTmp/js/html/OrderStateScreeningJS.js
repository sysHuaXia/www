...
function chk(TDID) {
    ...
    $.ajax({
        type: "POST",
        url: "/OrderStateScreening/StateScreeningAjax",
...
function BindSeachList(str) {
    ...
    for (var i = 0; i < orders.length - 1 ; i++) {
        ...
        tStr += '<td style="text-align:center;"><a href="/Order/UpdateOrderShow?oid=' + order[0] + '&version=' + (new Date()).getTime() + '" target="_blank" style="color:#6993E9;font-size:14px;" id="lsh_' + order[0] + '">' + order[1] + '</a></br>成交时间:' + order[13] + '<br/>' + order[16] + '</td>';
        ...
        tStr += '<td style="text-align:center;">' + order[5] + '人<br>' + order[8] + '</td>';
        tStr += '<td style="text-align:center;">' + order[7] + '</td>';
        tStr += '<td style="text-align:center;">' + order[9] + '</td>';
        tStr += '<td style="text-align:left;color:green;">' + order[14] + '</td>';
        tStr += '<td style="text-align:center;font-size:22px;">' + order[15] + '</td>';
        tStr += '<td ><input name="pp" readonly="readonly" type="checkbox" id="' + order[0] + '" lang="' + order[0] + '" value="' + order[0] + '"></td>';
        //if (order[17] == 6) {
        //    tStr += '<td ><input name="pp" readonly="readonly" type="checkbox" id="' + order[0] + '" lang="' + order[0] + '" value="' + order[0] + '"></td>';
        //} else {
        //    tStr += '<td ></td>';
...
//自动加载数据 
function BindStateScreeningData(OSatet, Screening, OCountry, Year, Month, Count) {
    if (OSatet == 7 && Screening == 0 && OCountry == 0 && Year == 0 && Month == 0) {
        $(".select-no").show();
    } else {
        $(".select-no").hide();
    }
