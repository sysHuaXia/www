...
function chhk(TDID) {
    $.ajax({
        type: "POST",
        url: "/OperationLog/IndexListAjax",
        data: $("#form1").serialize(),
        success: function (sesponseTest) {
            if (sesponseTest == "") {
                $("#t tbody").html("");
            } else {
                BindSeachLogList(sesponseTest);
            }
        }
    });
}
...
function BindSeachLogList(str) {
    ...
    for (var i = 0; i < orders.length - 1; i++) {
        ...
        tStr += '<td><a href="/Order/UpdateOrderShow?oid=' + order[1] + '&version=' + (new Date()).getTime() + '" target="_blank" id="lsh_' + order[1] + '">' + order[2] + '</a></td>';
        tStr += '<td>' + order[4] + '</td>';
        tStr += '<td>' + order[3] + '</td>';
        tStr += '</tr>';
    }
...
//自动加载数据
function BindLogListData(OSatet, Count) {
    if (OSatet == 0) {
        $(".select-no").show();
    } else {
        $(".select-no").hide();
