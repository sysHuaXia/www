...
Calendar.prototype.draw = function () {
    ...
    mvAry[mvAry.length] = '      <tr height="28px;">';
    ...
    mvAry[mvAry.length] = '        <th align="center" width="98%" nowrap="nowrap"><div align="center"><select name="calendarYear" id="calendarYear" style="font-size:12px;"></select>&nbsp;&nbsp;&nbsp;<select name="calendarMonth" id="calendarMonth" style="font-size:12px;';
    if (calendar.DateMode > pickMode["month"]) { mvAry[mvAry.length] = 'display:none;'; }//pickMode 精确到年时隐藏 “月” 
    ...
    mvAry[mvAry.length] = '      <tr height="25px;">';
    for (var i = 0; i < 7; i++) {
        mvAry[mvAry.length] = '      <th style="font-weight:normal;background-color:' + calendar.colors["tr_bg"] + ';color:' + calendar.colors["tr_word"] + ';">' + Calendar.language["weeks"][this.lang][i] + '</th>';
    }
    mvAry[mvAry.length] = '      </tr>';
    for (var i = 0; i < 6; i++) {
        mvAry[mvAry.length] = '    <tr align="center" height="25px;">';
        for (var j = 0; j < 7; j++) {
            if (j == 0) {
...
{
    var str = '<div id="calendarPanel" style="position: absolute;display: none;z-index:9999; background-color: #FFFFFF;border: 1px solid #CCCCCC;width:275px;font-size:12px;"></div>';
    if (document.all) {
