...
//终审
function ListAuditOrderCash(OrderID, CashID, UserID, OCType) {
    swal({
        title: "您确定要审核吗？",
        text: "您确定要审核当前记录？",
        type: "warning",
        ...
        $.ajax({
            ...
            data: "OrderID=" + OrderID + "&CashID=" + CashID + "&UserID=" + UserID + "&OCType=" + OCType,
            success: function (sesponseTest) {
                if (sesponseTest == "0") {
...
//初审
function AuditOrderCash(OrderID, CashID, UserID, OCType) {
    swal({
        title: "您确定要审核吗？",
        text: "您确定要审核当前记录？",
        type: "warning",
...
function BindSeachList(str,userType) {
    ...
    for (var i = 0; i < orders.length - 1 ; i++) {
        ...
        if (order[26] == "True") {
            tStr += '<div id="jk_' + order[1] + '" style="border:1px solid red; text-align:center;display:inline;">&nbsp;&nbsp;借款&nbsp;&nbsp;</div>';
        } else {
            tStr += '<div id="jk_' + order[1] + '" style="border:1px solid red; text-align:center;display:none;">&nbsp;&nbsp;借款&nbsp;&nbsp;</div>';
        }
        tStr += order[4] + '</td><td>供应商：' + order[19] + '<br />客户名：' + order[5] + '<br />销售：' + order[21] + '</td>';
        tStr += '<td style="text-align:center;">' + order[7] + '<span>人</span><br />&#165;<span id="je1_' + order[1] + '">' + toDecimal(order[9]) + '</span></td>'
        tStr += '<td>下单：' + formatDate(order[8], "yyyy-MM-dd") + '<br />支付：' + order[13] + '</td>';
        tStr += '<td>应收：&#165;<span id="ys_' + order[1] + '">' + toDecimal(order[9]) + '</span><br />实收：&#165;<span id="ss_' + order[1] + '">' + toDecimal(order[12]) + '</span><br />剩余：&#165;' + toDecimal(order[15]) + '<br />应付：&#165;<span id="yf_' + order[1] + '">' + toDecimal(order[10]) + '</span><br />实付：&#165;<span id="sf_' + order[1] + '">' + toDecimal(order[14]) + '</span></td>';
        ...
        if (order[18] == "3" || order[18] == "5" || order[18] == "6") {
            tStr += '<input id="stnOpen" type="button" value="补缴款项" onclick="opAdditionalPayment(' + order[1] + ');" class="btn-white btn btn-xs" />';
        }
        tStr += '</td>';
        tStr += '<td data-hide="all"><table class="footable table table-stripped toggle-arrow-tiny" id="' + order[1] + '"><thead><tr>';
                ...
                if (cash[18] == "True") {
                    ...
                    switch (cash[0]) {
                        ...
                        case "2":
                            ...
                            break;
                        case "3":
                            tStr += "<span style=\"color: red; \">借款</span>";
                            break;
                        default:
                            tStr += "<span style=\"color: green; \">还款</span>";
                            break;
                    }
                    tStr += '<span>[补缴]</span>';
                    tStr += '</td>';
                    ...
                    tStr += '</td>';
                    tStr += '<td align="center" id="status ' + cash[10] + '"></td>'
                    tStr += '<td align="center" id="csczy ' + cash[10] + '"></td>';
                    tStr += '<td align="center" id="cssj ' + cash[10] + '"></td>';
                    tStr += '<td align="center" id="fsczy ' + cash[10] + '"></td>';
                    tStr += '<td align="center" id="fssj ' + cash[10] + '"></td>';
                    tStr += '<td align="center"></td>';
                    tStr += '</tr>';
                } else {
                    tStr += '<tr id="tr_' + cash[10] + '" onmousemove="showPreview(this,event);" onmouseout="closePreview();">';
                    tStr += '<td>';
                    ...
                    if (cash[16] != "4" && UserType == "5")
                    {
                        if (cash[8] == "0")
                        {
                            tStr += "<input id=\"stnOpen\" type=\"button\" value=\"初审\" onclick=\"AuditOrderCash(" + cash[9] + "," + cash[10] + "," + UserID + "," + cash[16] + ")\" class=\"btn-white btn btn-xs\" />";
                        }
                        else if (cash[8] == "1")
                        {
                    ...
                    if (cash[16] != "4" && (UserType == "6" || UserType=="4")) {
                        if (cash[8] == "1") {
                            tStr += "<input id=\"stnOpen\" type=\"button\" value=\"终审\" onclick=\"ListAuditOrderCash(" + cash[9] + "," + cash[10] + "," + UserID + "," + cash[16] + ")\" class=\"btn-white btn btn-xs\" />";
                        }
                        else if (cash[8] == "2") {
                            tStr += "<input id=\"stnOpen\" type=\"button\" value=\"撤回\" onclick=\"ReListAuditOrderCash(" + cash[9] + "," + cash[10] + "," + UserID + ")\" class=\"btn-white btn btn-xs\" />";
                        }
                        else {
                            tStr += "<input id=\"stnOpen\" disabled=\"disabled\" type=\"button\" value=\"终审\" onclick=\"ListAuditOrderCash(" + cash[10] + "," + cash[9] + "," + UserID + "," + cash[16] + ")\" class=\"btn-white btn btn-xs\" />";
                        }
                    }
                    tStr += '</td>';
...
function opAdditionalPayment(OrderID) {
    $.webox({
        height: 290,
        width: 530,
        bgvisibel: true,
        title: '补缴款项',
        iframe: '/OrderCash/AdditionalPayment?OrderID=' + OrderID
    });
}

function getAdditionalPayment(str) {
    var arrStr = str.split(',');
    var strHtml = '<tr id="tr_' + arrStr[10] + '" onmousemove="showPreview(this,event);" onmouseout="closePreview();">';
    strHtml += '<td align="center">'
    if (arrStr[2] == "0") {
        strHtml += '<span style="color:green;">收款</span>'
        $("#je1_" + arrStr[0]).html(arrStr[13]);
        $("#ys_" + arrStr[0]).html(arrStr[13]);
        $("#ss_" + arrStr[0]).html(arrStr[13]);
    } else {
        strHtml += '<span style="color:dodgerblue;">支付</span>';
        $("#yf_" + arrStr[0]).html(arrStr[13]);
        $("#sf_" + arrStr[0]).html(arrStr[13]);
    }
    strHtml += '<span>[补缴]<span>';
    strHtml += '</td>';
    strHtml += '<td align="center">' + arrStr[3] + '</td>';
    strHtml += '<td align="center">' + arrStr[4] + '</td>';
    strHtml += '<td align="center" style="display:none;">' + arrStr[5] + '</td>';
    strHtml += '<td align="center">&#165;' + arrStr[6] + '</td>';
    strHtml += '<td align="left" width="268px" style="display:none;">' + arrStr[7] + '</td>';
    strHtml += '<td align="center">' + arrStr[8] + '</td>';
    strHtml += '<td align="center">' + arrStr[9] + '</td>';
    strHtml += '<td align="center">' + arrStr[9] + '</td>';
    strHtml += '<td align="center" id="pay ' + arrStr[10] + '"><span style="color:green;">已复审</span></td>';
    strHtml += '<td align="center" id="status ' + arrStr[10] + '"><input id="stnOpen" disabled="disabled" type="button" value="撤回" onclick="RetractOrderCash(' + arrStr[0] + ',' + arrStr[10] + ',' + arrStr[12] + ')" class="btn-white btn btn-xs" /></td>';
    strHtml += '<td align="center" id="csczy ' + arrStr[10] + '">' + arrStr[8] + '</td>';
    strHtml += '<td align="center" id="cssj ' + arrStr[10] + '">' + arrStr[9] + '</td>';
    strHtml += '<td align="center" id="fscz ' + arrStr[10] + '"><input id="stnOpen" type="button" value="撤回" onclick="ReListAuditOrderCash(' + arrStr[0] + ',' + arrStr[10] + ',' + arrStr[12] + ')" class="btn-white btn btn-xs" /></td>';
    strHtml += '<td align="center" id="fsczy ' + arrStr[10] + '">' + arrStr[8] + '</td>';
    strHtml += '<td align="center" id="fssj ' + arrStr[10] + '">' + arrStr[9] + '</td>';
    strHtml += '<td align="center"><input id="stnOpen" type="button" value="查看" onclick="opBill(' + arrStr[10] + ',0,0);" class="btn-white btn btn-xs" /></td>';
    strHtml += '</tr>';
    //$("#sc " + arrStr[0] + " tbody tr").append(strHtml);
    $("#" + arrStr[0] + " tbody").append(strHtml);
}