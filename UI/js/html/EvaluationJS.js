...
function chk(TDID) {
    ...
    $.ajax({
        type: "POST",
        url: "/OrderEvaluation/EvaluationAjax",
...
function BindSeachList(str) {
    ...
    for (var i = 0; i < orders.length - 1; i++) {
        ...
        tStr += '<td style="text-align:center;"><a href="/Order/UpdateOrderShow?oid=' + order[5] + '&version=' + (new Date()).getTime() + '" target="_blank" style="color:#6993E9;font-size:14px;" id="lsh_' + order[0] + '">' + order[1] + '</a></td>';
        tStr += '<td style="text-align:center;">' + order[2] + '</td>';
        tStr += '<td style="text-align:left;"><div class="divcss5">' + order[3] + '</div></td>';
...
//自动加载数据
function BindEvaluationData(OSatet,OCountry, Count) {
    if (OCountry == 0 || OSatet == 0) {
        $(".select-no").show();
    } else {
        $(".select-no").hide();
    }
