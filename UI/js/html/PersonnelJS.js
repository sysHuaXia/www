...
function BindSeachList(str) {
    ...
    for (var i = 0; i < orders.length - 1; i++) {
        ...
        tStr += '<td style="text-align:center;font-size:20px;">' + formatDate(order[12]) + '</td>';
        tStr += '<td style="text-align:center;font-size:14px;color:blue;">' + order[1] + '</td>';
        tStr += '<td style="text-align:center;">' + order[13] + '</td>';
        tStr += '<td style="text-align:left;"><span style="color:green">' + order[6] + '<br/>' + order[7] + '</span></td>';
        //tStr += '<td style="text-align:left;"><span style="color:#CCC" onclick="ShowTagExecutive(' + order[0] + ')">日志</span></td>';
        tStr += '</tr>';
...
//自动加载数据
function BindPersonnelData(OSatet, ResultState, OCountry, Count) {
    ...
    $.ajax({
        ...
        success: function (sesponseTest) {
            if (sesponseTest != "0") {
                ...
                var str = '<dt>休假国家：</dt><dd class="select-all selected"><a href="javascript:void(0);" onclick="Seach(0,2);">全部</a></dd>';
                var j = '';
...
function ShowTagExecutive(oid) {
    if (oid == '') {
        asyncbox.tips('标记', 'alert');
        return;
    }
    var winUrl = '/Executive/ShowTagExecutive?oid=' + oid + "&version=" + (new Date()).getTime();
    asyncbox.open({
        id: '123',
        title: '【新增】标记备注内容',
        url: winUrl,
