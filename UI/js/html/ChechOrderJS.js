...
function BindSeachList(str) {
    ...
    tStr += '<td height="30" bgcolor="#FF8E8E" width="50px"><div align="center">状态</div></td>';
    tStr += '<td height="30" bgcolor="#FF8E8E" width="150px"><div align="center">付款标签</div></td>';
    ...
    tStr += '<td height="30" bgcolor="#FF8E8E" width="8%"><div align="center">联系电话</div></td>';
    tStr += '<td height="30" bgcolor="#FF8E8E" width="8%"><div align="center">订单状态</div></td>';
    tStr += '<td height="30" bgcolor="#FF8E8E" width="8%"><div align="center">出签时间</div></td>';
    tStr += '<td height="30" bgcolor="#FF8E8E" width="8%"><div align="center">客人旺旺</div></td>';
    ...
    tStr += '<td height="30" bgcolor="#FF8E8E" width="8%" ><div align="center">收付状态</div></td>';
    tStr += '<td height="30" bgcolor="#FF8E8E" width="6%" ><div align="center">利润</div></td>';
    tStr += '</tr>';
    ...
    for (var i = 0; i < orders.length - 1 ; i++) {
        ...
        tStr += '<td style="' + order[11] + '" ></td>';
        tStr += '<td style="' + order[16] + '" ><div style="text-align:right;">' + order[15] + '</div></td>';
        
        tStr += '<td><a href="/Order/AddPaymentShow?oid=' + order[0] + '&version=' + (new Date()).getTime() + '" target="_blank" style="color:blue;font-size:14px;" id="lsh_' + order[0] + '">' + order[1] + '</a></td>';
        tStr += '<td>' + order[2] + ' (' + order[4] + '人)</td>';
        tStr += '<td>' + order[3] + '</td>';
        tStr += '<td>' + order[6] + '</td>';
        tStr += '<td>' + formatDate(order[5], "yyyy-MM-dd") + '</td>';
        tStr += '<td>' + order[7] + '<br><a target="_blank" href="http://www.taobao.com/webww/ww.php?ver=3&touid=' + order[7] + '&siteid=cntaobao&status=1&charset=utf-8"><img border="0" src="http://amos.alicdn.com/realonline.aw?v=2&uid=' + order[7] + '&site=cntaobao&s=1&charset=utf-8" alt="' + order[7] + '" /></a></td>';
        tStr += '<td>' + order[9] + '</td>';
        tStr += '<td>' + order[10] + '</td>';
        tStr += '<td><a href="/Order/AddPaymentShow?oid=' + order[0] + '&version=' + (new Date()).getTime() + '" target="_blank">' + order[12] + '/' + order[13] + '</a></td>';
        tStr += '<td>' + order[14] + '</td>';
        tStr += '</tr>';
    }
    $("#t tbody").html(tStr);
...
//自动加载数据  
function BindIniCheckData(OSatet, OType, Year, Month, AuditState, OProfits, Count) {
    var nowYear = new Date().getFullYear();
    var date = new Date;
    var month = date.getMonth() + 1;
    if (OSatet == 0 && OType == 0 && Year == nowYear && Month == month && AuditState == 0 && OProfits == 0) {
        $(".select-no").show();
    } else {
        $(".select-no").hide();
    }
...
function showYear() {
    ...
    var year3 = nowYear + 1;
    var jl = '';
    var str = '<dt>出签年份：</dt><dd><a href="javascript:void(0);" onclick="Seach(0,3);">全部</a></dd>';
...
function showData(startTime,endTime,orderAuditState) {
    ...
    var winUrl = '/CheckOrder/showDataList?startTime=' + startTime + '&endTime=' + endTime + '&orderAuditState=' + orderAuditState + "&version=" + (new Date()).getTime();
    asyncbox.open({
        id: '123',
        title: '数据列表',
        url: winUrl,
