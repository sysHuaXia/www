...
function BindSeachSmsList(str) {
    ...
    for (var i = 0; i < orders.length - 1 ; i++) {
        ...
        tStr += '<td>' + order[1] + '</td>';
        tStr += '<td>' + order[5] + '</td>';
        tStr += '<td><a href="/Order/UpdateOrderShow?oid=' + order[0] + '&version=' + (new Date()).getTime() + '" target="_blank" id="lsh_' + order[0] + '">' + order[2] + '</a></td>';

        tStr += '<td>';
        tStr += '<input id="ChkSelect' + order[0] + '" name="chkSon" type="checkbox" lang="' + order[5] + '" />';
        tStr += '<span ID="hfSupplierID" Value="' + order[0] + ',' + order[2] + ',0," style="display:none;"></span>';
        tStr += '</td>';
...
//自动加载数据
function BindSmsData(OSatet, Count) {
    if (OSatet == 0) {
        $(".select-no").show();
    } else {
        $(".select-no").hide();
